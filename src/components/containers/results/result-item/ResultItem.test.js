...
import {ResultItemPure as ResultItem} from './ResultItem';
...
describe('<ResultItem />', () => {
    ...
    it('should render free shipping icon', () => {
        const wrapper = shallow(<ResultItem t={key => key} item={mockItem}/>);
    ...
    it('should show new condition text', () => {
        const wrapper = shallow(<ResultItem t={key => key} item={mockItem}/>);
        expect(wrapper.find('.item-status').text()).toBe('new');
    });
   
    it('should show used condition text', () => {
        mockItem.condition = 'used';
        const wrapper = shallow(<ResultItem t={key => key} item={mockItem}/>);
        expect(wrapper.find('.item-status').text()).toBe('used');
    });
